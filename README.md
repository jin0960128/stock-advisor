...
2. 計算技術指標(RSI、MACD、均線等),分析線形趨勢
3. 用機器學習模型預測短期漲跌機率
4. 綜合以上三種訊號,給出「買入 X% / 觀望 X% / 賣出 X%」的建議
5. **把每次建議記錄下來**,持有天數到期後自動回顧「當初建議 vs 實際結果」
6. 長期累積後,統計出哪一組策略(權重組合)表現最好
...
- **🔍 分析**:輸入股票代號、選策略、按「開始分析」,結果(K線圖、新聞、建議百分比)都會直接顯示在網頁上
- **🔄 回顧更新**:按一個按鈕,自動回顧所有到期的建議
- **📊 績效統計**:表格呈現各策略的長期勝率與報酬率
- **🕘 歷史紀錄**:查詢某支股票過去的所有建議
...
- 把這次建議存進 `data/advisor_records.db`
